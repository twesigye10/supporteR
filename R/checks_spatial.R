  ...
  if(length(sample_pt_nos_thresh) > 0){
    ...
    df_data_with_distance %>%
      ...
      mutate(i.check.type = "remove_survey",
             ...
             i.check.issue = glue::glue("{distance} m greater_than_threshold: {input_threshold_dist} m"),
             i.check.other_text = "",
             i.check.checked_by = "",
             i.check.checked_date = as_date(today()),
             i.check.comment = "",
  ...
  if(length(sample_pt_nos_thresh) > 0){
    ...
    df_data_with_distance %>%
      ...
      mutate(i.check.change_type = "remove_survey",
             ...
             i.check.description = glue::glue("{distance} m greater_than_threshold: {input_threshold_dist} m"),
             i.check.other_text = "",
             i.check.comment = "",
             i.check.reviewed = "",
             i.check.so_sm_choices = "") %>%
